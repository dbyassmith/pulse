...
function validateEntry(entry: DateEntry, index?: number): string | null {
  ...
  if (!entry.title) return `${prefix}--title is required`;
  ...
  if (!entry.confidence) return `${prefix}--confidence is required`;
  if (!VALID_CONFIDENCE.includes(entry.confidence as (typeof VALID_CONFIDENCE)[number]))
...
export const dateCommand = new Command("date").description(
  "Manage confirmed dates"
);
